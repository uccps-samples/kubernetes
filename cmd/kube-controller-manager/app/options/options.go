...
func (s *KubeControllerManagerOptions) Flags(allControllers []string, disabledByDefaultControllers []string) cliflag.NamedFlagSets {
	...
	fs.StringVar(&s.OpenShiftContext.OpenShiftConfig, "uccp-config", s.OpenShiftContext.OpenShiftConfig, "indicates that this process should be compatible with openshift start master")
	fs.MarkHidden("uccp-config")
