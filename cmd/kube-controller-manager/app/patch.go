...
import (
	...
	libgorestclient "github.com/uccps-samples/library-go/pkg/config/client"
	"github.com/uccps-samples/library-go/pkg/monitor/health"
)
...
// newRejectIfNotReadyHeaderRoundTripper a middleware for setting X-uccp-Internal-If-Not-Ready HTTP Header for the given users.
...
func (rt *rejectIfNotReadyHeaderRT) RoundTrip(r *http.Request) (*http.Response, error) {
	...
	for _, eligibleUser := range rt.eligibleUsers {
		if strings.Contains(currentUser, eligibleUser) {
			r.Header.Set("X-uccp-Internal-If-Not-Ready", "reject")
			break
		}
	}
