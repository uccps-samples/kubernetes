	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			fakeRT := fakeRTFunc(func(r *http.Request) (*http.Response, error) {
				...
				headerSet := func() bool {
					if len(r.Header.Get("X-uccp-Internal-If-Not-Ready")) > 0 {
						return true
					}
					return false
				}()
				if scenario.expectHeader && !headerSet {
					return nil, fmt.Errorf("%v header wasn't set", textproto.CanonicalMIMEHeaderKey("X-uccp-Internal-If-Not-Ready"))
				}
				if !scenario.expectHeader && headerSet {
					return nil, fmt.Errorf("didn't expect %v header", textproto.CanonicalMIMEHeaderKey("X-uccp-Internal-If-Not-Ready"))
				}
				if scenario.expectHeader {
					if value := r.Header.Get("X-uccp-Internal-If-Not-Ready"); value != "reject" {
						return nil, fmt.Errorf("unexpected value %v in the %v header, expected \"reject\"", value, textproto.CanonicalMIMEHeaderKey("X-uccp-Internal-If-Not-Ready"))
					}
				}
				return nil, nil
			})
