...
func (opt *Options) Run(suite *TestSuite) error {
	...
	*/
	...
	if opt.PrintCommands {
		status := newTestStatus(opt.Out, true, len(tests), time.Minute, nil, nil, opt.AsEnv())
	...
	*/
	...
	status := newTestStatus(opt.Out, includeSuccess, expectedTestCount, timeout, nil, nil, opt.AsEnv())
	testCtx := ctx
	if opt.FailFast {
	...
	*/
	...
	if fail > 0 && fail <= suite.MaximumAllowedFlakes {
		...
		q := newParallelTestQueue()
		status := newTestStatus(ioutil.Discard, opt.IncludeSuccessOutput, len(retries), timeout, nil, nil, opt.AsEnv())
	...
	if len(opt.JUnitDir) > 0 {
		if err := writeJUnitReport("junit_e2e", "kube-tests", tests, opt.JUnitDir, duration, opt.ErrOut); err != nil {
