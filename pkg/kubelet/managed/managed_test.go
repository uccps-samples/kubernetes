...
func TestStaticPodManaged(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedAnnotations: map[string]string{
				"target.workload.uccp.io/management": `{"effect": "PreferredDuringScheduling"}`,
				"resources.workload.uccp.io/nginx":   `{"cpushares":102}`,
			},
		},
		{
			pod: &v1.Pod{
			...
			expectedAnnotations: map[string]string{
				"target.workload.uccp.io/management": `{"effect": "PreferredDuringScheduling"}`,
				"resources.workload.uccp.io/c1":      `{"cpushares":102}`,
				"resources.workload.uccp.io/c2":      `{"cpushares":1024}`,
				"resources.workload.uccp.io/c_3":     `{"cpushares":1024}`,
			},
		},
	}
	...
	}{
		{
			pod: &v1.Pod{
				...
				ObjectMeta: metav1.ObjectMeta{
					...
					Namespace: "mynamespace",
					Annotations: map[string]string{
						"target.workload.uccp.io/throttle": `{"effect": "PreferredDuringScheduling"}`,
					},
				},
				Spec: v1.PodSpec{
					Containers: []v1.Container{
