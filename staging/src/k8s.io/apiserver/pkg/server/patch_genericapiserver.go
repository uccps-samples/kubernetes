	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if late {
			if pth := "/" + strings.TrimLeft(r.URL.Path, "/"); pth != "/readyz" && pth != "/healthz" && pth != "/livez" {
				if isLocal(r) {
					audit.AddAuditAnnotation(r.Context(), "uccp.io/during-graceful", fmt.Sprintf("loopback=true,%v,readyz=false", r.URL.Host))
					...
					audit.AddAuditAnnotation(r.Context(), "uccp.io/during-graceful", fmt.Sprintf("loopback=false,%v,readyz=false", r.URL.Host))
		...
		if pth := "/" + strings.TrimLeft(r.URL.Path, "/"); pth != "/readyz" && pth != "/healthz" && pth != "/livez" {
			if isLocal(r) {
				if !isKubeApiserverLoopBack(r) {
					audit.AddAuditAnnotation(r.Context(), "uccp.io/unready", fmt.Sprintf("loopback=true,%v,readyz=false", r.URL.Host))
				...
				audit.AddAuditAnnotation(r.Context(), "uccp.io/unready", fmt.Sprintf("loopback=false,%v,readyz=false", r.URL.Host))
