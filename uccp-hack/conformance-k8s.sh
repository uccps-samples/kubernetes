...
This conformance report is generated by the OpenShift CI infrastructure. The canonical source location for this test script is located at https://github.com/uccps-samples/kubernetes/blob/master/uccp-hack/conformance-k8s.sh
...
3. Clone the OpenShift source repository and change to that directory:
    ...
    git clone https://github.com/uccps-samples/kubernetes.git
    cd kubernetes

4. Place the \`oc\` binary for that cluster in your PATH
5. Run the conformance test:

    uccp-hack/conformance-k8s.sh

Nightly conformance tests are run against release branches and reported https://uccp-gce-devel.appspot.com/builds/origin-ci-test/logs/periodic-ci-origin-conformance-k8s/
END
