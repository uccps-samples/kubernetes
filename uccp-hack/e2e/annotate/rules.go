...
import (
	...
	_ "k8s.io/kubernetes/uccp-hack/e2e"
)
...
var (
	TestMaps = map[string][]string{
		...
		"[Disabled:Broken]": {
			...
			// A fix is in progress: https://github.com/uccps-samples/origin/pull/24709
		...
		"[Skipped:gce]": {
			...
			// We are using uccp-sdn to conceal metadata
		...
		// Tests that don't pass under uccp-sdn.
		// These are skipped explicitly by uccp-hack/test-kubernetes-e2e.sh,
		// but will also be skipped by uccp-tests in jobs that use uccp-sdn.
		"[Skipped:Network/OpenShiftSDN]": {
			`NetworkPolicy.*IPBlock`,    // feature is not supported by uccp-sdn
			`NetworkPolicy.*[Ee]gress`,  // feature is not supported by uccp-sdn
			`NetworkPolicy.*named port`, // feature is not supported by uccp-sdn
