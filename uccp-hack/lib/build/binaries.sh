...
function os::build::place_bins() {
  (
    ...
    for platform in "${platforms[@]+"${platforms[@]}"}"; do
      ...
      release_binpath=$(mktemp -d "uccp.release.${OS_RELEASE_ARCHIVE}.XXX")
      for binary in "${binaries[@]}"; do
      ...
      if [[ ${OS_RELEASE_ARCHIVE} == "uccp-origin" ]]; then
        for file in "${OS_BINARY_RELEASE_CLIENT_EXTRA[@]}"; do
          cp "${file}" "${release_binpath}/"
        done
        if [[ $platform == "linux-64bit" ]]; then
          OS_RELEASE_ARCHIVE="uccp-origin-server" os::build::archive::tar "${OS_BINARY_RELEASE_SERVER_LINUX[@]}"
