...
# https://github.com/uccps-samples/kubernetes/blob/master/REBASE.openshift.md#preparing-the-local-repo-clone
...
# validate input args --k8s-tag=v1.21.2 --uccp-release=release-4.8 --bugzilla-id=2003027
k8s_tag=""
openshift_release=""
bugzilla_id=""
...
usage() {
  ...
  echo "  --uccp-release  (required) Example: --uccp-release=release-4.8"
  echo "  --bugzilla-id        (optional) creates new PR against openshift/kubernetes:${uccp-release}: Example: --bugzilla-id=2003027"
}
...
if [ -z "${openshift_release}" ]; then
  echo "Required argument missing: --uccp-release"
  echo ""
  usage
  exit 1
fi
...
# fetch remote https://github.com/uccps-samples/kubernetes
...
# uccp-hack/images/hyperkube/Dockerfile.rhel still has FROM pointing to old tag
# we need to remove the prefix "v" from the $k8s_tag to stay compatible
sed -i -E "s/(io.openshift.build.versions=\"kubernetes=)(1.[1-9]+.[1-9]+)/\1${k8s_tag:1}/" uccp-hack/images/hyperkube/Dockerfile.rhel
go_mod_go_ver=$(grep -E 'go 1\.[1-9][0-9]?' go.mod | sed -E 's/go (1\.[1-9][0-9]?)/\1/')
tag="rhel-8-release-golang-${go_mod_go_ver}-uccp-${openshift_release#release-}"
