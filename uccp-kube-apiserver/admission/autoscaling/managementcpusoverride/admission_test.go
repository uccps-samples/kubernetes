...
const (
	...
	managedCapacityLabel = "management.workload.uccp.io/cores"
)
...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			namespace:     testManagedNamespace(),
			nodes:         []*corev1.Node{testNodeWithManagementResource()},
			expectedError: fmt.Errorf("the pod without workload annotations can not have containers with workload resources %q", "management.workload.uccp.io/cores"),
		},
		{
