...
import (
	...
	v1 "k8s.io/kubernetes/uccp-kube-apiserver/admission/autoscaling/apis/runonceduration/v1"
	"k8s.io/kubernetes/uccp-kube-apiserver/admission/autoscaling/apis/runonceduration/validation"
)

func Register(plugins *admission.Plugins) {
	plugins.Register("autoscaling.uccp.io/RunOnceDuration",
		func(config io.Reader) (admission.Interface, error) {
			...
			if pluginConfig == nil {
				klog.Infof("Admission plugin %q is not configured so it will be disabled.", "autoscaling.uccp.io/RunOnceDuration")
				return nil, nil
			}
...
func (a *runOnceDuration) ValidateInitialization() error {
	if a.nsLister == nil {
		return errors.New("autoscaling.uccp.io/RunOnceDuration plugin requires a namespace listers")
	}
	return nil
}
