...
const PluginName = "config.uccp.io/DenyDeleteClusterConfiguration"
...
func (p *admissionPlugin) Validate(ctx context.Context, attributes admission.Attributes, _ admission.ObjectInterfaces) error {
	...
	if attributes.GetResource().Group != "config.uccp.io" {
		return nil
	}
	...
	// other config.uccp.io resources not named `cluster` can be deleted (none are expected to exist).
