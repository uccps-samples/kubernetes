...
import (
	...
	"github.com/uccps-samples/apiserver-library-go/pkg/labelselector"
)

func Register(plugins *admission.Plugins) {
	plugins.Register("scheduling.uccp.io/OriginPodNodeEnvironment",
...
func (p *podNodeEnvironment) admit(ctx context.Context, a admission.Attributes, mutationAllowed bool) (err error) {
	...
	if !p.waitForSyncedStore(time.After(timeToWaitForCacheSync)) {
		return admission.NewForbidden(a, errors.New("scheduling.uccp.io/OriginPodNodeEnvironment: caches not synchronized"))
	}
