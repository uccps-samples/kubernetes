...
import (
	...
	"k8s.io/kubernetes/uccp-kube-apiserver/admission/scheduler/apis/podnodeconstraints"
)
...
func TestPodNodeConstraints(t *testing.T) {
	...
	tests := []struct {
		...
		// 4: expect nodeSelector to succeed with user that has "pods/binding" access
		{
			config:           testConfig(),
			resource:         nodeSelectorPod(),
			userinfo:         serviceaccount.UserInfo("uccp-infra", "daemonset-controller", ""),
...
func (a *fakeTestAuthorizer) Authorize(_ context.Context, attributes authorizer.Attributes) (authorizer.Decision, string, error) {
	...
	if ui.GetName() == "system:serviceaccount:uccp-infra:daemonset-controller" {
...
func TestReadConfig(t *testing.T) {
	configStr := `apiVersion: scheduling.uccp.io/v1
