...
const ServiceCACertConfigMapName = "uccp-service-ca.crt"
...
func getInjectionAnnotation() map[string]string {
	setAnnotationOnce.Do(func() {
		...
		useVulnerable := os.Getenv("UCCP_USE_VULNERABLE_LEGACY_SERVICE_CA_CRT")
		...
		if useVulnerableBool {
			...
			injectionAnnotation = map[string]string{
				"service.alpha.uccp.io/inject-vulnerable-legacy-cabundle": "true",
			}
		}
	})
